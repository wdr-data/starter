...
const Marker = ({ x, y, textLines, color, edge, hidden }) => {
  ...
  const width =
    Math.max(...textLines.map((text) => String(text).length)) * 8 + 25;
  const margin = {
    bottom: 18,
    right: 0,
  };
...
const YDILineInternal = ({ name }) => {
  const question = useMemo(() => require(`../../../data/ydi/${name}.json`), [
    name,
  ]);
  ...
  const height = useMemo(() => (isMobile ? width * 0.9 : width * 0.75), [
    width,
  ]);
  ...
  const notAllData = useMemo(() => knownData.concat(unknownData), [
    knownData,
    unknownData,
  ]);
  ...
  const dragX = useMemo(() => xScale(x(lastKnown)) + margin.left, [
    xScale,
    lastKnown,
    margin,
  ]);
  ...
  const guessCallback = useCallback(
    (e, force) => {
      ...
      if (!isDragging && !hasGuessed) {
        setPreviewTarget({
          x: xScale(x(firstUnknown)) - xScale.step() / 2,
          y: yPos,
        });
      }
      ...
      const effectiveLabel =
        unknownData.findIndex((d) => d.label === label) !== -1
          ? label
          : unknownData[0].label;
      ...
      setGuessProgress(
        Math.max(
          guessProgress,
          unknownData.findIndex((d) => d.label === effectiveLabel),
        ),
      );
      setGuesses(
        guesses.map((guess, i) =>
          unknownData[i].label === effectiveLabel ? newGuess : guess,
        ),
      );
    },
    [
  ...
  const groupUnknown = useMemo(() => {
    return (
      <Group top={margin.top} left={margin.left}>
        ...
        <LinePath
          data={[{ ...lastKnown, guess: lastKnown.value }]
            .concat(guessData)
            .slice(0, guessProgress !== null ? guessProgress + 2 : 0)}
          x={(d) => xScale(x(d))}
          y={(d) => yScale(yGuess(d))}
      ...
      <Group top={margin.top} left={margin.left}>
        ...
        <circle
          ...
          className={classNames(
            styles.marker,
            !confirmAnimationDone && styles.hidden,
          )}
        />
